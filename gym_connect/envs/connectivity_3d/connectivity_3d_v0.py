...
from gym_connect.utils.lattice3d import *
from gym_connect.utils.k3d_connectivity_controller import KConnectivityController
...
class Connectivity3DV0(gym.Env):
    ...
    def reset(self,i=0):
        ...
        if self.mode != 'keyboard':
            ...
            self.end_node = [self.end_node[0],self.end_node[1],0]
        ...
        self.x[0,:3] = self.start_node
        self.x[1,:3] = self.end_node
    ...
    def step(self, v):
        ...
        and internal states based on the given velocity commands.

        Parameters:
        v (ndarray: shape(n_agents,3)): Desired velocity commands for each UAV (n_agents, nu).
        ...
        self.x[0, :3], self.x[1, :3] = self.start_node, self.end_node
    ...
    def controller(self):
        '''
        Compute and return the combined control input for connectivity and repulsion.
        '''
    ...
    def get_curve(self):
        ...
        self.path_xy = np.array([[x, y, 0] for (x, y) in self.path_xy])
    ...
    def set_position(self,pos,idx):        
        self.x[idx,:3] = np.array(pos)   
        # if we are updating start and end nodes
        if idx == 1:        
            self.end_node = self.x[1,:3]
        if idx == 0:        
            self.start_node = self.x[0,:3]
