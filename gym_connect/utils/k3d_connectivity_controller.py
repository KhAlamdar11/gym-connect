...
class KConnectivityController:
    ...
    def __call__(self, position, batteries):
        ...
        positionx, positiony, positionz, ac, eigV = self.set_controlData(position)
            ...
            for j in range(nv):
                ...
                if A[i, j] > 0:
                    ...
                    if self.params['battery_aware']:
                        ...
                        dotz[i] += k * (positionz[i] - positionz[j]) #* batt_gain
                    else:
                        dotx[i] += k * (positionx[i] - positionx[j]) 
                        doty[i] += k * (positiony[i] - positiony[j]) 
                        dotz[i] += k * (positionz[i] - positionz[j])
   
        # connectivity control part         
        u_c = np.column_stack([dotx, doty, dotz]) * self.params['gainConnectivity']
    ...
    def set_controlData(self, position):
        ...
        return position[:, 0], position[:, 1], position[:, 2], np.array([ac]), np.array(eigV)
    ...
    def battery_gain(self,b):
        return np.exp((self.critical_battery_level - b) / self.battery_tau) + 1
